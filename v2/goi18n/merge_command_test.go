...
func TestMergeWithLeanPrettify(t *testing.T) {
	testCases := []*testCase{
		{
			name:           "single identity",
			sourceLanguage: language.AmericanEnglish,
			...
			outFiles: map[string][]byte{
				"active.en-US.toml": []byte(`[1HelloMessage]
other = "Hello"
`),
			},
		},
...
`),
			...
			outFiles: map[string][]byte{
				"active.en-US.toml": []byte(`[Body]
other = "Finally some text!"
`),
			},
		},
		{
...
`),
			...
			outFiles: map[string][]byte{
				"active.en-US.toml": expectFile(`[Body]
other = "Some text!"
`),
			},
		},
...
`),
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			args := append([]string{"merge", "-pretty", "lean", "-sourceLanguage", testCase.sourceLanguage.String(), "-outdir", outdir}, infiles...)
			if code := testableMain(args); code != 0 {
				t.Fatalf("expected exit code 0; got %d\n", code)
			}
...
func TestMergeWithFullPrettify(t *testing.T) {
	testCases := []*testCase{
		{
			name:           "single identity",
			sourceLanguage: language.AmericanEnglish,
...
`),
			...
			outFiles: map[string][]byte{
				"active.en-US.toml": []byte(`[Body]
hash = "sha1-f2564ffe3f77a5588c54c250630b5a455e2484d8"
other = "Finally some text!"
`),
			},
		},
...
`),
			...
			outFiles: map[string][]byte{
				"active.en-US.toml": []byte(`[Body]
hash = "sha1-712d777ff40108c01a1040bb2b1751df8b86e083"
other = "Some text!"
`),
			},
		},
...
[Footer]
hash = "sha1-1d72170161230587bab04efbe64a6d37a0911854"
other = "Some more text!"
`),
			},
		},
...
[everything]
few = "few translation"
hash = "sha1-7fdef56f291878cd18022d73a56d38dc5b2dc749"
many = "many translation"
one = "one translation"
other = "other translation"
two = "two translation"
...
[simple]
hash = "sha1-0ff190867de64148cdaec785366d7584ed5f9f55"
other = "simple translation"
`),
			},
		},
...
`),
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			args := append([]string{"merge", "-pretty", "full", "-sourceLanguage", testCase.sourceLanguage.String(), "-outdir", outdir}, infiles...)
			if code := testableMain(args); code != 0 {
				t.Fatalf("expected exit code 0; got %d\n", code)
			}
