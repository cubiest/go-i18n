...
func TestExtractWithLeanPrettify(t *testing.T) {
...
`),
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			indir := mustTempDir("TestExtractWithPrettyLeanCommandIn")
			defer os.RemoveAll(indir)
			outdir := mustTempDir("TestExtractWithPrettyLeanCommandOut")
			defer os.RemoveAll(outdir)
			...
			if code := testableMain([]string{"extract", "-pretty", "lean", "-outdir", outdir, indir}); code != 0 {
				t.Fatalf("expected exit code 0; got %d\n", code)
			}
...
func TestExtractWithPrettyLeanCommand(t *testing.T) {
	outdir, err := ioutil.TempDir("", "TestExtractWithPrettyLeanCommand")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(outdir)
	if code := testableMain([]string{"extract", "-pretty", "lean", "-outdir", outdir, "../example/"}); code != 0 {
		t.Fatalf("expected exit code 0; got %d", code)
	}
	actual, err := ioutil.ReadFile(filepath.Join(outdir, "active.en.toml"))
	if err != nil {
...
func TestExtractWithPrettyFullCommand(t *testing.T) {
	outdir, err := ioutil.TempDir("", "TestExtractWithPrettyFullCommand")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(outdir)

	t.Run("unsupported pretty flag value", func(t *testing.T) {
		expected := 1
		actual := testableMain([]string{"extract", "-pretty", "full", "-outdir", outdir, "../example/"})
